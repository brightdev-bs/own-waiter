...
public class OwnWaiterApplication {
	...
	void init() {
		...
		User admin = User.builder().email("manager@gmail.com").password("123").authorities(new ArrayList<GrantedAuthority>(Arrays.asList(new SimpleGrantedAuthority("ADMIN")))).build();
		User user = User.builder().email("user@gmail.com").password("123").authorities(new ArrayList<GrantedAuthority>(Arrays.asList(new SimpleGrantedAuthority("USER")))).build();
